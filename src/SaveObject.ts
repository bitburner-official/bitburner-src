...
/**
 * This function asserts the unknown saveObject. It's called from "loadGame", and contains validation logic originally
 * from that function. Please note that this logic is flawed, so I made some changes.
 *
 * In "loadGame", we parse a json save string to saveObject, then load data from this object. When we do that, we have
 * to ensure that this object contains valid data. The validation logic in "loadGame" was really lenient. This is how
 * "loadGame" did it (I'll change these behaviors in the implementation of this function if needed):
 * - Some properties of the object are "mandatory". We assume that they are strings without performing any checks.
 * - Some properties are "optional":
 *   - "Optional 1": If saveObject does not have it, we will only print a warning, then use an empty string as a
 * fallback value; otherwise, we assume that it's a string without performing any checks, then pass it to a "loader". If
 * the "loader" fails to load the (assumed) string value, we call the "loader" with an empty string.
 *   - "Optional 2": If saveObject has it, we assume that it's a string without performing any checks.
 */
function assertBitburnerSaveObjectType(saveObject: unknown): asserts saveObject is BitburnerSaveObjectType {
  throwErrorIfNotObject(saveObject);

  // These keys are "mandatory". We assert that the values are strings.
  const mandatoryKeysOfSaveObj = ["PlayerSave", "AllServersSave", "CompaniesSave", "FactionsSave"];
  for (const key of mandatoryKeysOfSaveObj) {
    const value = saveObject[key];
    if (typeof value !== "string") {
      throw new Error(`Save data does not have ${key}.`);
    }
  }

  /**
   * These keys are "Optional 1". If the values are not strings, we print warnings and use an empty string as the
   * fallback value.
   *
   * Note that, in most cases, using an empty string as the default value is nonsense. Most loaders use JSON.parse() to
   * parse this string value, and JSON.parse will throw an error if you pass an empty string to it. To avoid making a
   * massive change, which is hard to test and review, I'll not change this behavior now.
   */
  const optional1KeysOfSaveObj = ["StaneksGiftSave", "AliasesSave", "GlobalAliasesSave", "StockMarketSave"];
  ...
  /**
   * These keys are "Optional 2". If saveObject has them, we assert that they are strings.
   */
  ...
  for (const key of optional2KeysOfSaveObj) {
    ...
    if (value != null && typeof value !== "string") {
      throw new Error(`Save data has ${key}, but it's not a string.`);
    }
  }
}
