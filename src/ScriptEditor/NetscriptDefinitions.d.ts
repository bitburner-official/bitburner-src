...
export interface CodingContract {
   ...
   * RAM cost: 0 GB
   *
   * Attempts to get the expected answer of a conding contract. Throws an exception for some contract types.
   * Converts the contract into a dummy contract with no reward (like the contracts generated by
   * {@link CodingContract.createDummyContract | createDummyContract}).
   *
   * @param filename - Filename of the contract.
   * @param host - Hostname/IP of the server containing the contract. Optional. Defaults to current server if not provided.
   * @returns The specified contract’s answer, data type depends on contract type.
   */
...
export type CodingContractObject = {
  [T in keyof CodingContractSignatures]: {
    ...
    getAnswer: () => CodingContractSignatures[T][1];
