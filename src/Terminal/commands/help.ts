...
// This section is named "I don't think I know how to use TypeScript".
      ...
      try {
        ...
        compile(script, localServer.scripts).then((compiledModule) => {
          ...
          if (typeof helpObj === "string") {
            ...
            // ReactNode[] is in the ReactNode type. Lol.
            if (isStringArray(helpObj)) {
              const flushedOut: string = helpObj.join("\n");
            ...
            if (isHelpData(helpObj)) {
              const autogeneratedHelp: string = generateHelpText(helpObj, script, localServer).join("\n");
              Terminal.print(autogeneratedHelp);
              return;
            }
            Terminal.printRaw(wrapUserNode(helpObj));
          }
