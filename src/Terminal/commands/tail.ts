...
// TODO-FICO: support piping out of tail
export function tail(commandArray: (string | number | boolean)[], server: BaseServer, stdIO: StdIO): void {
  try {
    if (commandArray.length < 1) {
      Terminal.error("Incorrect number of arguments. Usage: tail [pid] or tail [scriptname] [arg1] [arg2]...", stdIO);
    } else if (typeof commandArray[0] === "string") {
      const [rawName, ...args] = commandArray;
      const path = Terminal.getFilepath(rawName);
      if (!path) return Terminal.error(`Invalid filename: ${rawName}`, stdIO);
      if (!hasScriptExtension(path))
        return Terminal.error(`Invalid file extension. Tail can only be used on scripts.`, stdIO);

      const candidates = findRunningScripts(path, args, server);

      // if there's no candidate then we just don't know.
      if (candidates === null) {
        Terminal.error(`No script named ${path} with args ${JSON.stringify(args)} is running on the server`, stdIO);
        return;
      }
      ...
      if (runningScript == null) {
        Terminal.error(`No script with PID ${commandArray[0]} is running`, stdIO);
        return;
      }
