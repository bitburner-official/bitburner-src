...
export function cp(args: (string | number | boolean)[], server: BaseServer, stdIO: StdIO): undefined {
  ...
  if (!sourceFilePath) return Terminal.fatal(`Invalid source filename ${args[0]}`, stdIO);
  if (!hasTextExtension(sourceFilePath) && !hasScriptExtension(sourceFilePath)) {
    return Terminal.fatal("cp: Can only be performed on script and text files", stdIO);
  }
  const source = server.getContentFile(sourceFilePath);
  if (!source) return Terminal.fatal(`File not found: ${sourceFilePath}`, stdIO);
  ...
  if (!destFilePath) {
    ...
    if (!destDirectory)
      return Terminal.fatal(`Could not resolve ${destinationInput} as a FilePath or Directory`, stdIO);
  ...
  if (!hasTextExtension(destFilePath) && !hasScriptExtension(destFilePath)) {
    return Terminal.fatal(`cp: Can only copy to script and text files (${destFilePath} is invalid destination)`, stdIO);
  }
