...
export function mv(args: (string | number | boolean)[], server: BaseServer): void {
  ...
  if (!sourceContentFile.deleteFromServer(server)) {
    return Terminal.error(`Could not remove source file ${sourcePath} from existing location. If ${sourcePath} is a script, make sure that it is NOT running before trying to use 'mv' on it.`);
  }
