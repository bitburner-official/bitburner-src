...
export function NetscriptFormulas(): InternalAPI<IFormulas> {
  ...
  const formulasFunctions: InternalAPI<IFormulas> = {
    ...
    hacking: {
      ...
      weakenEffect: (ctx, _threads, _cores = 1) => {
        const threads = helpers.number(ctx, "threads", _threads);
        const cores = helpers.number(ctx, "cores", _cores);
        checkFormulasAccess(ctx);
    ...
    hacknetNodes: {
      ...
      hacknetNodeCost: (ctx, _n, _mult = 1) => {
    ...
    dnet: {
      getAuthenticateTime: (ctx, _darknetServerDetails, _threads, _player, _correctCharactersInPassword): number => {
      ...
      getHeartbleedTime: (ctx, _darknetServerDetails, _threads, _player): number => {
        assertDarknetServerDetails(ctx, _darknetServerDetails);
        const threads = helpers.number(ctx, "threads", _threads ?? 1);
        const person = helpers.person(ctx, _player ?? Player);
        return calculateAuthenticationTime(_darknetServerDetails, person, threads) * 1.5;
      },
      getExpectedRamBlockRemoved: (ctx, _darknetServerDetails, _threads, _person): number => {
        assertDarknetServerDetails(ctx, _darknetServerDetails);
        const threads = helpers.number(ctx, "threads", _threads ?? 1);
