...
import { formatBigNumber, nFormat } from "../../ui/nFormat";
...
function WarehouseRoot(props: IProps): React.ReactElement {
  ...
  for (const matName of Object.values(corpConstants.materialNames)) {
    ...
    breakdownItems.push(
      <>
        {matName}: {nFormat(mat.qty * MaterialInfo[matName].size, "0,0.0")}
      </>,
    );
  }
  ...
  for (const prodName of Object.keys(division.products)) {
    ...
    breakdownItems.push(
      <>
        {prodName}: {nFormat(prod.data[props.warehouse.loc][0] * prod.siz, "0,0.0")}
      </>,
    );
  }
