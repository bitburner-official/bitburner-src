...
export function Overview({ rerender }: IProps): React.ReactElement {
  ...
  function appendMult(name: string, value: number): void {
    if (value === 1) return;
    multRows.push([name, nFormat(value, "0.000")]);
  }
    ...
    <>
      <StatsTable
        rows={[
          ...
          ["Owned Stock Shares:", nFormat(corp.numShares, "0.000a")],
      ...
      <Box display="flex">
        <Tooltip
          title={
            <StatsTable
              rows={[
                ["Outstanding Shares:", nFormat(corp.issuedShares, "0.000a")],
                ["Private Shares:", nFormat(corp.totalShares - corp.issuedShares - corp.numShares, "0.000a")],
              ]}
            />
          }
        >
          <Typography>Total Stock Shares: {nFormat(corp.totalShares, "0.000a")}</Typography>
        </Tooltip>
      </Box>
      <br />
  ...
  return (
    <StatsTable
      rows={[
        ...
        ["Dividend Percentage:", nFormat(corp.dividendRate, "0%")],
