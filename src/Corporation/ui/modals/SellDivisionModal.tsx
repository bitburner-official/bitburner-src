...
export function SellDivisionModal(props: IProps): React.ReactElement {
  ...
  function sellDivision(){
    ...
    dialogBoxCreate(
      <>
        Sold {industry.name} for {formatMoney(price)}, you now have space for {corp.maxDivisions-corp.divisions.length} more divisions.
      </>,
    );
  }
  ...
  return (
    <Modal open={props.open} onClose={props.onClose}>
      ...
      <Typography>
        ...
        <br></br>
        - Warehouses: {Object.keys(industry.warehouses).map(city => !!industry.warehouses[city as CityName]?1:0).reduce(sum,0)}
        {industry.makesProducts??<Typography> <br></br>- Products: {industry.products.length} </Typography>}
        <br></br>
        <br></br>
