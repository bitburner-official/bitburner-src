...
function BitNodeModifiedStats(props: IBitNodeModifiedStatsProps): React.ReactElement {
  ...
  if (props.mult === 1 || Player.sourceFileLvl(5) === 0)
    return <Typography color={props.color}>{formatPercentage(props.base)}</Typography>;

  return (
    <Typography color={props.color}>
      <span style={{ opacity: 0.5 }}>{formatPercentage(props.base)}</span> {formatPercentage(props.base * props.mult)}
    </Typography>
  );
}
