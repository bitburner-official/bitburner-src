...
export function CashIn(): React.ReactElement {
  ...
  function processTickets(): void {
    ...
    for (const l649ticket of l649) {
      l649correct = 0;
      l649bonus = false;
      for (let num of l649ticket.Numbers) {
    ...
    let boxbet = false;
    let waybet = 0;
    for (const p3ticket of pick3Straight) { // Straight first.
      if (p3ticket.Numbers[0] === pick3WinNums[0] && p3ticket.Numbers[1] === pick3WinNums[1] && p3ticket.Numbers[2] === pick3WinNums[2]) {
    ...
    for (const p3ticket of pick3Box) { // Box Next.
      waybet = whichWayP3(p3ticket.Numbers); // 3 or 6 way bet
      if (waybet === 3 && isP3BoxWinner(p3ticket.Numbers, 3)) { // Process a 3-way Box ex: 445
        pick3Winnings += p3ticket.Wager * 160;
      }
      else if (waybet === 6 && isP3BoxWinner(p3ticket.Numbers, 6)) { // Process a 6-way Box  ex: 456
        pick3Winnings += p3ticket.Wager * 40;
      }
    }

    for (const p3ticket of pick3StraightBox) { // Straight-Box Next.
      waybet = whichWayP3(p3ticket.Numbers); // 3 or 6 way bet
      if (waybet === 3 && isP3StraightBoxExactWinner(p3ticket.Numbers, 3)) { // Process a 3-way Straight-Box ex: 445
      ...
      else if (waybet === 6 && isP3StraightBoxExactWinner(p3ticket.Numbers, 6)) { // Process a 6-way Box  ex: 456
        pick3Winnings += p3ticket.Wager * 290;
      }
      else if (waybet === 6 && isP3StraightBoxAnyOrderWinner(p3ticket.Numbers, 6)) { // Process a 6-way Box  ex: 456
        pick3Winnings += p3ticket.Wager * 40;
      }
    }

    // Pick 4 ticket grouping
    for (const p4ticket of pick4Straight) { // Straight first.
      if (p4ticket.Numbers[0] === pick4WinNums[0] && p4ticket.Numbers[1] === pick4WinNums[1] && p4ticket.Numbers[2] === pick4WinNums[2] && p4ticket.Numbers[3] === pick4WinNums[3]) {
    ...
    for (const p4ticket of pick4Box) { // Box Next.
      waybet = whichWayP4(p4ticket.Numbers);
      if (waybet === 4 && isP4BoxWinner(p4ticket.Numbers, 4)) { // Process a 4-way Box
        pick4Winnings += p4ticket.Wager * 1200;
      }
      else if (waybet === 6 && isP4BoxWinner(p4ticket.Numbers, 6)) { // Process a 6-way Box
        pick4Winnings += p4ticket.Wager * 800;
      }
      else if (waybet === 12 && isP4BoxWinner(p4ticket.Numbers, 12)) { // Process a 12-way Box
        pick4Winnings += p4ticket.Wager * 400;
      }
      else if (waybet === 24 && isP4BoxWinner(p4ticket.Numbers, 24)) { // Process a 24-way Box
  ...
  function processWinnings(): void {
    if (l649Winnings + kenoWinnings + pick2Winnings + pick3Winnings + pick4Winnings + Player.getLotteryWinnings() > LotteryConstants.GainLimits) {
        dialogBoxCreate("An error occured with the lottery system.  We could not process all winnings.  We are sorry for the inconvenience.");
    }
  ...
  function pullBall(): number {
    const ball = balls.splice(balls.length * BadRNG.random(), 1)
  ...
  function isP3BoxWinner(numarray: number[], way: number): boolean {
    if (way === 3) { // Process a 3-way box
      let winnums: number[] = [];
      winnums.push(...pick3WinNums);
      winnums.sort((a, b) => {return b - a })
      numarray.sort((a, b) => { return b - a })
      while (numarray.length > 0) {
        let num = numarray.pop();
        let winnum = winnums.pop();
        if (num === winnum) {
          continue
        }
        else return false;
    ...
    else { // Process a 6-way box
      if (pick3WinNums.includes(numarray[0]) && pick3WinNums.includes(numarray[1]) && pick3WinNums.includes(numarray[2])) {
        return true;
      }
      else {
        return false;
  ...
  function isP4BoxWinner(numarray: number[], way: number): boolean {
    if (way === 24) { // Process a 24-way box
      if (pick4WinNums.includes(numarray[0]) && pick4WinNums.includes(numarray[1]) && pick4WinNums.includes(numarray[2]) && pick4WinNums.includes(numarray[3])) {
        return true;
      }
      else {
        return false;
  ...
  function isP3StraightBoxExactWinner(numarray: number[], way: number): boolean {
  ...
  const br = <Typography> <br /> </Typography>;
  const l649display = <Typography> Lotto 6/49:
    &nbsp;{l649WinNums.pop()}, {l649WinNums.pop()}, {l649WinNums.pop()}, {l649WinNums.pop()}, {l649WinNums.pop()},  {l649WinNums.pop()} Bonus:  {l649WinNumsBonus.pop()}</Typography>;

  const pick2display = <Typography>Pick 2: 
    &nbsp;{pick2WinNums.pop()}, {pick2WinNums.pop()}</Typography>;

  const pick3display = <Typography>Pick 3:
    &nbsp;{pick3WinNums.pop()}, {pick3WinNums.pop()}, {pick3WinNums.pop()}</Typography>;

  const pick4display = <Typography>Pick 4:
    &nbsp;{pick4WinNums.pop()}, {pick4WinNums.pop()}, {pick4WinNums.pop()}, {pick4WinNums.pop()}</Typography>

  const kenodisplay = <Typography>Keno: &nbsp;{kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}<br />
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}<br />
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}<br />
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}, {kenoWinNums.pop()}</Typography>

  const l649windisp = l649Winnings > 0 ? <Typography>Lotto 6/49 Winnings from {l649.length}x tickets: ${formatNumber(l649Winnings)}</Typography> : <Typography>Lotto 6/49 Winnings from {l649.length}x tickets : $0</Typography>;
  const pick2windisp = pick2Winnings > 0 ? <Typography>Lotto Pick2 Winnings from {pick2.length}x tickets: ${formatNumber(pick2Winnings)}</Typography> : <Typography>Lotto Pick2 Winnings from {pick2.length}x tickets : $0</Typography>
  const pick3windisp = pick3Winnings > 0 ? <Typography>Lotto Pick3 Winnings from {totalPick3()}x tickets: ${formatNumber(pick3Winnings)}</Typography> : <Typography>Lotto Pick3 Winnings from {totalPick3()}x tickets : $0</Typography>
  const pick4windisp = pick4Winnings > 0 ? <Typography>Lotto Pick4 Winnings from {totalPick4()}x tickets: ${formatNumber(pick4Winnings)}</Typography> : <Typography>Lotto Pick4 Winnings from {totalPick4()}x tickets : $0</Typography>
  const kenowindisp = kenoWinnings > 0 ? <Typography>Lotto Keno Winnings from {keno.length}x tickets: ${formatNumber(kenoWinnings)}</Typography> : <Typography>Lotto Keno Winnings from {keno.length}x tickets : $0</Typography>
