...
export function LotteryStoreLocation(): React.ReactElement {
  ...
  const GiveIceCreamButton =
    IceCream() && Player.bitNodeN === 15 && girlwithicecream == false ? (
      <Button onClick={() => GiveIceCream()}>Give Ice Cream</Button>
    ) : (
      ""
    );
  const IceCreamGirl =
    Player.bitNodeN === 15 && girlwithicecream === false ? (
      <Typography>
        <i>A little girl is here, wishing she had some Ice Cream...</i>
      </Typography>
    ) : Player.bitNodeN === 15 && girlwithicecream ? (
      <Typography>
        <i>A little girl is here, eating her Ice Cream cone</i>
      </Typography>
    ) : (
      ""
    );
  ...
  const WomanGreeting = (
    <Typography>
      <i>A friendly woman beckons you through the door. "Come on in!".</i>
    </Typography>
  );
