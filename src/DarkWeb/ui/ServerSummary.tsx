...
export function ServerSummary({server, enableAuth}: ServerSummaryProps): React.ReactElement {
  ...
  const runningScriptCount = server.runningScriptMap.values().map((pidMap: Map<number, RunningScript>) => pidMap.size).reduce((a, b) => a + b, 0);


  return (
    <Container className={classes.inlineFlexBox}>
      {cacheCount ? <Tooltip title={<>Reward cache count: {cacheCount}</>}>
        <Typography><SvgIcon component={Inventory2} className={`${classes.gold} ${classes.paddingRight}`} />{cacheCount}</Typography>
      </Tooltip> : ""}
      {contractCount ? <Tooltip title={<>Coding contract count: {contractCount}</>}>
        <Typography><SvgIcon component={Code} className={classes.paddingRight} />{contractCount}</Typography>
      </Tooltip> : ""}
      <Tooltip title={<>Data files on server: {fileCount}</>}>
        <Typography><SvgIcon component={Description} className={classes.paddingRight} />{fileCount}</Typography>
      </Tooltip>
      <Tooltip title={<>Running scripts on server: {runningScriptCount}</>}>
        <Typography><SvgIcon component={Terminal} className={classes.paddingRight} />{runningScriptCount}</Typography>
      </Tooltip>
    </Container>
  );
}