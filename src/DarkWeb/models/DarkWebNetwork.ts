...
export const addRandomConnections = (server: DarkWebServer) => {
  ...
  horizontalNeighbors.forEach((neighbor) => {
    if (Math.random() < HORIZONTAL_CONNECTION_CHANCE) {
      server.connections.push(neighbor.id);
      neighbor.connections.push(server.id);
    }
  });

  const serversAbove = getServersOnColumnAbove(x, y);
  const serversBelow = getServersOnColumnBelow(x, y);
  [...serversAbove, ...serversBelow].forEach((server) => {
    const distance = Math.abs(server.x - x);
    if (Math.random() < VERTICAL_CONNECTION_CHANCE / distance) {
      server.connections.push(server.id);
      server.connections.push(server.id);
    }
  });
}
...
export const getNeighborsOnRow = (x: number, y: number): DarkWebServer[] => {
  ...
  const leftNeighbor = DarkWebNetwork[x - 1]?.[y];
  const rightNeighbor = DarkWebNetwork[x + 1]?.[y];
  if (leftNeighbor) {
...
export const getServersOnColumnBelow = (x: number, y: number): DarkWebServer[] => {
  return DarkWebNetwork[y-1]?.filter(notNull<DarkWebServer>) ?? [];
}

export const getServersOnColumnAbove = (x: number, y: number): DarkWebServer[] => {
  return DarkWebNetwork[y+1]?.filter(notNull<DarkWebServer>) ?? [];
}
