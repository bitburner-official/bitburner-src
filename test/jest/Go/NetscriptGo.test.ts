...
import { GoOpponent, GoColor, GoPlayType } from "../../../src/Go/Enums";
...
describe("Netscript Go API unit tests", () => {
  describe("makeMove() tests", () => {
    it("should handle invalid moves", async () => {
      const board = ["XOO..", ".....", ".....", ".....", "....."];
      Player.go.boardState = getBoardFromSimplifiedBoardState(board, GoOpponent.Daedalus, GoColor.white);
    ...
    it("should update the board with valid player moves", async () => {
      ...
      expect(Player.go.boardState.board[1]?.[0]?.player).toEqual(GoColor.black);
      expect(Player.go.boardState.board[0]?.[0]?.player).toEqual(GoColor.empty);
    });
  });
  ...
  describe("resetBoardState() tests", () => {
    it("should set the player's board to the requested size and opponent", () => {
      ...
      expect(Player.go.boardState.ai).toEqual(GoOpponent.SlumSnakes);
    });
...
*/
  ...
  describe("cheatPlayTwoMoves() tests", () => {
    ...
    it("should update the board with both player moves if nodes are unoccupied and cheat is successful", async () => {
      ...
      expect(result.success).toEqual(true);
      expect(Player.go.boardState.board[4]?.[3]?.player).toEqual(GoColor.black);
      expect(Player.go.boardState.board[3]?.[4]?.player).toEqual(GoColor.black);
      expect(Player.go.boardState.board[4]?.[4]?.player).toEqual(GoColor.empty);
    });
    ...
    it("should pass player turn to AI if the cheat is unsuccessful but player is not ejected", async () => {
      ...
      expect(Player.go.boardState.board[4]?.[3]?.player).toEqual(GoColor.empty);
      expect(Player.go.boardState.board[3]?.[4]?.player).toEqual(GoColor.empty);
      expect(Player.go.boardState.board[4]?.[4]?.player).toEqual(GoColor.white);
    });
