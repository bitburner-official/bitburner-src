...
|  [memoryReallocation(hostname)](./bitburner.darknet.memoryreallocation.md) | Spends some time freeing some of the RAM currently blocked by the server owner. Must target a directly connected server. The amount of ram so absconded scaled with charisma and the number of threads used. |
